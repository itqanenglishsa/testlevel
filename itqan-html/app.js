  ...
  return `
    <article class="card" style="background:var(--card);color:var(--foreground)">
      <div class="flex items-center gap-2">
        <span class="lvl-pill lvl-pill-sm">${c.level}</span>
        <span class="muted text-sm">${c.category[lang]}</span>
      </div>

      <h3 class="text-lg font-semibold mt-3">${c.title[lang]}</h3>
    ...
    c.key === "vocab500"
      ...
      <div class="mt-5 flex items-center justify-between" style="border-top:1px solid var(--border);padding-top:1rem">
        <span class="font-bold">
          <img src="../images/SARIcon.png" alt="SAR" class="currency-icon">
...
// ==========================================================================
/* منطق تشغيل واجهات التسجيل والدخول داخل النافذة المنبثقة (مع القيود الكاملة) */
// ==========================================================================

// دالة معرفة لغة الموقع الحالية
function popGetLanguage() {
...
// دالة جلب النصوص البديلة والرسائل التحذيرية الصارمة للغتين
function popGetText(key) {
  const currentLang = popGetLanguage();
  
  const translations = {
    ar: {
      createAccountTitle: "إنشاء الحساب",
      ...
      googleSign: "تسجيل الدخول بواسطة Google",
      appleSign: "تسجيل الدخول بواسطة Apple",
      loginTitle: "تسجيل الدخول",
      btnLogin: "تسجيل الدخول",
      resetTitle: "إعادة تعيين كلمة المرور",
      enterEmailPlace: "أدخل بريدك الإلكتروني",
      btnReset: "إرسال رابط إعادة التعيين", // تم التصحيح هنا ✅
      backToReg: "العودة لصفحة التسجيل",
      // رسائل الأخطاء والقيود الصارمة
      errEmailEmpty: "يرجى إدخال عنوان بريدك الإلكتروني.",
      errEmailInvalid: "صيغة البريد الإلكتروني غير صحيحة.",
      errPassEmpty: "يرجى إدخال كلمة المرور.",
      errPassRequirements: "كلمة المرور: 8+ خانات، حرف كبير واحد، رقم واحد، رمز واحد.",
      errConfirmEmpty: "يرجى تأكيد كلمة المرور الخاصة بك.",
      errMismatch: "كلمات المرور غير متطابقة. يرجى التأكد من تطابق الحقلين.",
      successCreate: "تم إنشاء حسابك بنجاح. أهلاً بك!",
      successLogin: "تم تسجيل الدخول بنجاح.",
      successReset: "تم إرسال رابط إعادة التعيين إلى بريدك الإلكتروني." // تم التصحيح هنا ✅
    },
    en: {
      ...
      appleSign: "Sign in with Apple",
      loginTitle: "Login",
      btnLogin: "Login",
      resetTitle: "Reset Password",
      enterEmailPlace: "Enter your email",
      btnReset: "Send reset link",
      backToReg: "Back to registration",
      // رسائل الأخطاء والقيود الصارمة
    errEmailEmpty: "Please enter your email address.",
      errEmailInvalid: "The email address format is invalid.",
      errPassEmpty: "Please enter your password.",
      errPassRequirements: "Password: 8+ characters, 1 uppercase letter, 1 number, 1 symbol.",
      errConfirmEmpty: "Please confirm your password.",
      errMismatch: "Passwords do not match. Please ensure both fields are identical.",
      successCreate: "Your account has been created successfully. Welcome!",
      successLogin: "Logged in successfully.",
      successReset: "A reset link has been sent to your email."
    }
  };
...
// دالة الإشعار المخصص
function popShowAlert(message) {
  const existingAlert = document.getElementById("custom-alert");
  if (existingAlert) existingAlert.remove();
  
  const alertDiv = document.createElement("div");
  alertDiv.id = "custom-alert";
  alertDiv.style.cssText = `position: fixed; top: 20px; left: 50%; transform: translateX(-50%) translateY(-20px); background-color: #fff; color: #0078D4; border: 2px solid #0078D4; padding: 14px 24px; border-radius: 25px; box-shadow: 0 4px 15px rgba(0, 120, 212, 0.15); font-family: "Segoe UI", sans-serif; font-size: 15px; font-weight: 600; line-height: 1.5; z-index: 1000000; white-space: pre-line; max-width: 90%; width: max-content; text-align: center; opacity: 0; transition: transform 0.3s ease, opacity 0.3s ease; backdrop-filter: blur(4px);`;
  alertDiv.textContent = message;
  document.body.appendChild(alertDiv);
  
  setTimeout(() => { alertDiv.style.transform = "translateX(-50%) translateY(0)"; alertDiv.style.opacity = "1"; }, 10);
  setTimeout(() => { alertDiv.style.transform = "translateX(-50%) translateY(-20px)"; alertDiv.style.opacity = "0"; setTimeout(() => alertDiv.remove(), 300); }, 4500);
}

// بناء فورم إنشاء الحساب
...
function popLoadLoginForm() {
  ...
  container.innerHTML = `
    ...
    <div class="auth-links" style="margin-top: 20px; font-size: 14px; color: #666;">
      <a href="#" id="back-register" style="color: #0078D4; text-decoration: none; margin: 0 5px;">${popGetText("btnCreate")}</a> |
...
function popLoadForgotForm() {
  ...
  container.innerHTML = `
    ...
    <button class="auth-btn" id="reset-btn" style="width: 100%; padding: 14px; margin-top: 10px; border-radius: 25px; border: none; background: #0078D4; color: #fff; font-size: 16px; font-weight: 600; cursor: pointer;">${popGetText("btnReset")}</button>
...
function popIsValidStrictEmail(email) {
  // 1. منع أي مسافات نهائياً داخل حقل الإيميل
  if (/\s/.test(email)) return false;

  // 2. التحقق من التنسيق والامتداد (ألا يقل عن 3 حروف)
  const emailRegex = /^[^\s@]+@[^\s@]+\.[a-zA-Z]{3,}$/;
  if (!emailRegex.test(email)) return false;

  // 3. عزل النطاق للتحقق من الشركات الكبرى الحقيقية (Whitelisting)
